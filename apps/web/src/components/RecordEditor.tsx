    ...
    <form onSubmit={handleSubmit} className="flex flex-wrap items-end gap-3">
      <div className="space-y-1">
        ...
        <select
          ...
          className="block rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground"
        >
          {RECORD_TYPES.map((t) => (
      ...
      <div className="space-y-1">
        ...
        <input
          ...
          className="block w-24 rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground"
          min={60}
        />
      </div>
      <button
        type="submit"
        className="cursor-pointer rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90"
      >
        {submitLabel}
      </button>
    </form>
