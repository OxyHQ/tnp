...
export function startEcosystemActivity(ready: () => boolean): void {
  if (!process.env.OXY_SERVICE_API_KEY?.trim() || !process.env.OXY_SERVICE_API_SECRET?.trim()) {
    console.warn('Ecosystem activity is disabled for tnp-relay (missing OXY_SERVICE_API_KEY/OXY_SERVICE_API_SECRET)');
    return;
  }
  if (activity) return;
  activity = createEcosystemTraffic({ service: 'tnp-relay', ready });
